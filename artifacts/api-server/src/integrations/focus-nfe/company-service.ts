...
export async function getFocusCompanySummary(storeId: number) {
  ...
  const certificateConfigured = Boolean(settings?.certificateReference && settings.certificateStatus === "valid");
  const cscConfigured = Boolean(settings?.cscId && settings?.cscSecretReference);
  const missingRequirements = await getHomologationMissingRequirements(storeId, settings, homologationCredentialConfigured, certificateConfigured, cscConfigured);
  return {
...
function certMeta(providerCompanyId: string, data: FocusCompanyUpdateResponse) {
  ...
  return { reference: `focus_company:${providerCompanyId}:certificate`, status: "valid", expiresAt: expiresRaw && !Number.isNaN(Date.parse(expiresRaw)) ? new Date(expiresRaw) : null };
}
...
export async function uploadFocusCertificate(input: { storeId: number; actorUserId?: number | null; filename: string; content: Buffer; password: string; client?: FocusNfeClient }) {
  ...
  if (!context.providerCompanyId) throw new Error("Empresa Focus NFe ainda não vinculada à loja.");
  const previous = await getFocusCompanySummary(input.storeId);
  await audit(db, input.storeId, input.actorUserId, "focus_certificate_uploaded", context.providerCompanyId, { environment: "homologation", status: "submitted" });
  try {
    const result = await (input.client ?? new FocusNfeClient()).request<FocusCompanyUpdateResponse>(context, { method: "PUT", path: `/v2/empresas/${encodeURIComponent(context.providerCompanyId)}`, body: { arquivo_certificado_base64: input.content.toString("base64"), senha_certificado: input.password } });
    ...
    await db.transaction(async (tx) => {
      ...
      await audit(tx, input.storeId, input.actorUserId, "focus_certificate_validated", context.providerCompanyId!, { environment: "homologation", status: meta.status, certificateExpiresAt: meta.expiresAt?.toISOString() ?? null });
    ...
    throw new Error("A Focus NFe rejeitou ou não validou o certificado. A configuração anterior foi mantida.");
...
function validateCertificate(filename: string, content: Buffer, password: string) {
  ...
  if (content.length === 0) throw new Error("O certificado enviado está vazio.");
  if (content.length > 5 * 1024 * 1024) throw new Error("O certificado deve ter no máximo 5 MB.");
