...
const paymentMap: Record<string,string> = { cash:"01", credit_card:"03", debit_card:"04", voucher:"10", pix:"17" };
...
export async function buildHomologationNfcePayload(input:{ db: DbExecutor; storeId:number; orderId:number; series:number; number:number }) {
  ...
  const payments = await executor.select().from(paymentsTable).where(and(eq(paymentsTable.orderId, orderId), eq(paymentsTable.status, "approved")));
  ...
  const rules = await executor.select().from(productFiscalSettingsTable).where(and(eq(productFiscalSettingsTable.storeId, storeId), inArray(productFiscalSettingsTable.productId, productIds)));
  const products = await executor.select({ id: productsTable.id, name: productsTable.name, storeId: productsTable.storeId }).from(productsTable).where(and(eq(productsTable.storeId, storeId), inArray(productsTable.id, productIds)));
  ...
  if (mode === "complete") {
    produtos = items.map((i,idx)=>{ const r=byProduct.get(i.productId!); if(!r || !isFiscalRuleComplete(r)) throw new NfceServiceError("PRODUCT_FISCAL_RULE_MISSING", "Produto sem regra fiscal completa."); const desc=[productNames.get(i.productId!) ?? i.externalProductName, i.variantName].filter(Boolean).join(" - "); return fiscalLine(idx+1, i.productId!, desc, i.quantity, cents(i.totalPrice), r); });
  } else {
    ...
    const groupRules = await executor.select().from(fiscalGroupRulesTable).where(and(eq(fiscalGroupRulesTable.storeId, storeId), inArray(fiscalGroupRulesTable.fiscalGroupId, groupIds)));
    const groups = await executor.select({ id:fiscalGroupsTable.id, name:fiscalGroupsTable.name }).from(fiscalGroupsTable).where(and(eq(fiscalGroupsTable.storeId, storeId), inArray(fiscalGroupsTable.id, groupIds)));
    ...
    produtos = [...grouped.entries()].map(([gid,g],idx)=>fiscalLine(idx+1, gid, groupNames.get(gid)!, g.q, g.total, byGroup.get(gid)!));
  }
